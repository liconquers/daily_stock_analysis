...
- [改进] 每日分析、上游同步和网络冒烟失败时通过 Telegram 主动告警；Network Smoke 至少要求一条验证路径成功，不再无条件显示绿色。
...
- [修复] 上游同步改用具备仓库级 `Contents` 与 `Workflows` 写权限的专用令牌，修复上游更新包含 GitHub Actions 文件时默认令牌拒绝推送的问题。
